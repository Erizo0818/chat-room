...
public:
  void exec(void *login_task_data) override {
    ...
    auto iter = users.find(message->get_sender());
    const char *password = iter != users.end() ? iter->second : nullptr;

    if (password != nullptr && strcmp(password, (char *)message->get_data()) == 0) {
      pthread_rwlock_wrlock(server->get_sender_fd_rwlock());
      server->get_sender_fd()->insert({message->get_sender(), fd});
      std::cout<<"insert: ("<<message->get_sender()<<", "<<fd<<")"<<std::endl;
