...
uint32_t CRPMessage::get_sender() const {
    return sender;
}
uint32_t CRPMessage::get_receiver() const {
    return receiver;
}

const uint8_t *CRPMessage::get_data() const {
    return data;
}
uint8_t CRPMessage::get_op_code() const {
    return op_code;
}
...
int CRP::receive(CRPMessage *message) {
    int n = recv(fd, recv_buf + recv_pointer, 4096 - recv_pointer, 0);  //如果接收成功，返回实际接收到的字节数。
